# Newest version of the dashboard used to display graphs
...
def graphV6(request):
  ...
  filterOptions['windowEndAtDate'] = request.GET.get('windowEndAtDate', datetime.today().strftime("%Y-%m-%d"))
  if filterOptions['windowEndAtDate'] == '':
    filterOptions['windowEndAtDate'] = datetime.today().strftime("%Y-%m-%d")
  ...
  if(filteredDataRows['PacketV6']):
    filteredDataRows['BallastStatusString'] = uc.ballast_status_conv(filteredDataRows['PacketV6'].ballast_status)
    filteredDataRows['CutdownStatusString'] = uc.cutdown_status_conv(filteredDataRows['PacketV6'].cutdown_status)
    if(IMEI == 'None'):
